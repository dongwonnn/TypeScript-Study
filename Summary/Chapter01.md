...
JS의 기능적 문제로 인한 오류의 원인을 알 수 있어 코드를 수월하게 작성 가능합니다.
이러한 이점 때문에 대규모 소프트웨어 개발에 이용됩니다.

#### 트랜스파일

- 트랜스파일 : 어떤 프로그래밍 언어로 작성된 소스코드를 또 다른 프로그래밍 언어로 된 소스코드로 바꿔주는 프로그램. 컴파일러와는 다릅니다.
  1. 바벨 : ESNext를 ES5로 변환.
  2. TSC : TypeScript를 ES5로 변환.

### 1-2. 타입스크립트 주요 문법

- 타입스크립트는 ESNext 문법을 지원합니다.
...
- Promise와 async/await 구문
  ...
  `async` : 함수 수정자
  `await` : `async` 함수 본문에서 사용가능. Promise 객체를 resolve 해 준다. 따라서 then을 이용해 값을 얻을 수 있습니다.
...
- 타입 주석과 타입 추론
  1. 타입 주석 : 콜론(:)과 타입 이름
     `let n : number = 1`
  2. 타입 추론 : 타입 주석은 생략 가능하다. = 연산자의 오른쪽 값을 분석해 왼쪽 변수의 타입을 결정할 수 있습니다. 이 기능으로 인해 확장자 js를 ts로 바꿔도 타입스크립트 환경에서 바로 동작 가능합니다.
     `let m = 2`
- 인터페이스
...
- 튜플
  물리적으론 배열과 같습니다. 배열에 저장되는 아이템의 데이터 타입이 모두 같으면 배열, 다르면 튜플.
  `let array : number[] = [1,2,3] `
  `let tuple : [boolean, number, string] = [true, 1, 'OK']`
- 제네릭 타입
  다양한 타입을 한꺼번에 취급할 수 있게 해줍니다. number, string, array, boolean 등..
...
- 이후에 node hello.js를 실행시키면 실행됩니다.

#### ts-node 설치

- 타입스크립트 코드를 ES5로 변환하고 실행까지 동시에 할 수 있습니다.
- `npm i -g ts-node`
- `ts-node hello.ts`
